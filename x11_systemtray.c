...
void rearrange_icons() {
    ...
    for (IconNode *node = icon_list; node != NULL; node = node->next) {
        XMoveWindow(display, node->icon_window, x, (TRAY_HEIGHT - ICON_SIZE) / 2);
        x += ICON_SIZE + ICON_PADDING;
    }
}

void handle_systray_message(XEvent *event) {
    if (event->type == ClientMessage && event->xclient.message_type == _NET_SYSTEM_TRAY_OPCODE) {
...
int main(void) {
    ...
    tray_window = XCreateSimpleWindow(display, root, TRAY_X, TRAY_Y, TRAY_WIDTH, TRAY_HEIGHT, 1,
    ...
    // Select input events
    XSelectInput(display, tray_window, ExposureMask | ButtonPressMask | StructureNotifyMask);
    ...
    tray_selection_window = XCreateSimpleWindow(display, root, 0, 0, 1, 1, 0, 0, 0);

    // Claim the system tray selection
    XSetSelectionOwner(display, _NET_SYSTEM_TRAY_S, tray_selection_window, CurrentTime);
    if (XGetSelectionOwner(display, _NET_SYSTEM_TRAY_S) != tray_selection_window) {
